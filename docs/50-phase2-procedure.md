...
1. **[진행] 로봇 설정 마무리** — 연결·차폐 검증 완료 ✅. 카메라: bringup에서 분리(§4c), `/image_raw`를 PC에서 직접 처리하는 구조로 확정 ✅.
...
## 4c. 기동 구조 확정 (2026-07-02 갱신)
- **이미지 처리 = PC (로봇 아님).** 재부팅 후 원본 `/image_raw`가 PC에 **14.3Hz 직접 수신**됨.
  compressed는 fresh rclpy 노드(=처리 노드)에서 discovery 매칭이 안 돼 PC 0Hz → **raw 직접 구독으로 확정**
  (실측 근거: `docs/60-network-workaround.md`). `camera_obstacle_node.py` 입력부를
  CompressedImage/imdecode → **원본 Image/numpy reshape(rgb8→BGR)**로 전환, PC에서 14.8Hz 처리·`/camera_obstacles` 발행 검증.
- **카메라를 bringup에서 분리** (2026-07-02): 로봇 `turtlebot3_bringup/launch/robot.launch.py`의
  `camera` 인자 기본값 `true`→`false`. 이제 로봇 기동은 **2단계**:
    1. `ros2 launch turtlebot3_bringup robot.launch.py`   — 베이스(카메라 없음)
    2. `ros2 launch turtlebot3_bringup camera.launch.py`  — v4l2_camera → `/image_raw`
  함께 띄우려면 `robot.launch.py camera:=true`. (install이 src symlink라 리빌드 불필요, 백업 `robot.launch.py.bak_precamera_split`)
  ⚠️ 재기동 시 `camera.launch.py`를 따로 안 띄우면 `/image_raw`가 없어 **PC 처리 노드가 이미지 못 받음**.
- **처리 노드 위치**: 로봇에서 돌던 `camera_obstacle_node` 중지 → 이제 **PC에서 실행**
  (현재 수동 백그라운드, 상시화는 PC측 launch 통합이 TODO).
- 접속: `ssh ubuntu@192.168.0.189` (키 등록됨, 무암호).

## 6. 미확정/질문
- ✅ (해결) **카메라 종류**: RGB 단독(IMX219 CSI). ROS2 `v4l2_camera`로 발행(`/image_raw`, `/camera_info`), 640×480 rgb8.
- ✅ (해결) **기동 방식**: `turtlebot3_bringup camera.launch.py`의 v4l2_camera_node (§4c). 이 로봇은 libcamera 미지원이라 v4l2 스택 사용.
