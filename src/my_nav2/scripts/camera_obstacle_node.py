...
의존: cv2(system), numpy. (cv_bridge 불필요 — /image_raw/compressed 를 cv2.imdecode)
"""
import math
import struct
...
class CameraObstacle(Node):
    def __init__(self):
        ...
        # --- 토픽/프레임 ---
        self.in_topic = p('image_topic', '/image_raw/compressed').value
        ...
        # --- 바닥 HSV 모델 (실측 파란카펫: H92-118 & S>60, V 폭넓게) ---
        self.h_lo = p('floor_h_lo', 95).value
        self.h_hi = p('floor_h_hi', 113).value
        self.s_lo = p('floor_s_lo', 38).value
        self.s_hi = p('floor_s_hi', 255).value
        self.v_lo = p('floor_v_lo', 30).value
        self.v_hi = p('floor_v_hi', 235).value
        # --- 검출 파라미터 ---
        ...
        self.create_subscription(CompressedImage, self.in_topic, self.image_cb, qos_profile_sensor_data)
    ...
    def image_cb(self, msg):
        ...
        for u in range(0, W, self.col_step):
            ...
            elif run < self.min_floor_run and col[H-1] == 0:
                # 바로 앞이 이미 장애물 (로봇 코앞)
                contact_v = H - 1
